...
> **상태: 설계 완료, 미구현.** 아래 `현행 확정사항`이 실행 기준이며, 이후 장문 섹션은 구현 근거와 상세 계약을 제공한다.

## 현행 확정사항

Logging Hook으로 실제 이벤트 형식을 운영 검증한 뒤 `PreToolUse`에 최소한의 위험 명령 차단 규칙을 추가한다. 이 Guard는 반복되는 실수를 줄이는 보조 통제이며 Codex 샌드박스나 운영체제 권한을 대체하는 보안 경계가 아니다.
...
`PreToolUse` payload만으로 과거 사용자 승인을 신뢰성 있게 판정할 수 없으므로 승인 여부를 명령 문자열에서 추측하지 않는다. 경로가 변수, glob 또는 명령 치환으로 모호하면 안전하게 차단한다. 필요한 운영 작업은 Guard 밖의 수동 절차나 별도 승인 인터페이스로 수행한다.
...
Logging Hook의 실제 턴 검증 후 별도 TDD 구현 계획을 작성한다. Hybrid RAG와 동시에 구현하지 않는다.
...
- 실제 Codex `PreToolUse`에서 안전한 명령은 실행되고 위험 명령은 실행 전에 차단된다.
